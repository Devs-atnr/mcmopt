...
class CollectionFiltersForm extends HTMLElement {
    ...
    static renderPriceFilter(priceFilterBlock) {
        if (priceFilterBlock) {
            ...
            const applyButton = htmlElement.querySelector('[id^="filter__price--apply"]');
            if (applyButton) {
                applyButton.addEventListener('click', debouncedOnClick);
            }

            const priceRange = htmlElement.querySelector('price-range');
            if (priceRange && typeof priceRange._initWhenReady === 'function') {
                priceRange._initialized = false;
                requestAnimationFrame(() => priceRange._initWhenReady(true));
            }
        }
    }
...
static renderPage(searchParams, event, updateURLHash = true) {
    // Capture the current grid layout
    const currentGridLayout = document.querySelector('.icon-mode.active')?.dataset.col || '2'; // Default to 2 columns if no layout is selected
    sessionStorage.setItem('selectedGridLayout', currentGridLayout);
...
static renderProductGrid(html) {
    ...
    const innerHTML = parsedHTML
        .getElementById('CollectionProductGrid')
        ?.querySelector('.collection')?.innerHTML;

    const gridElement = document.getElementById('CollectionProductGrid')?.querySelector('.collection');
    if (gridElement && innerHTML != null) {
        gridElement.innerHTML = innerHTML;
    }

    const resultsCount = parsedHTML.querySelector('.results-count .count')?.textContent?.trim();
    if (resultsCount) {
        document.querySelectorAll('.results-count .count').forEach((el) => {
            el.textContent = resultsCount;
        });
        document.querySelectorAll('.results-count .results').forEach((el) => {
            el.style.display = '';
        });
    }

    const selectedGridLayout = sessionStorage.getItem('selectedGridLayout') || '2';
    this.initViewModeLayout(parseInt(selectedGridLayout, 10));
}
    ...
    static handleClick(event) {
        ...
        const formEl = event.target.closest('collection-filters-form');
        if (formEl && typeof formEl.onConvertPrice === 'function') {
            searchParams = formEl.onConvertPrice(searchParams);
        }
    ...
    static renderActiveFacets(html) {
        ...
        activeFacetElementSelectors.forEach((selector) => {
            ...
            const refineBlock = document.querySelector(selector);
            if (refineBlock) refineBlock.innerHTML = activeFacetsElement.innerHTML;
...
class PriceRange extends HTMLElement {
    ...
    connectedCallback() {
        this._initWhenReady();
        document.addEventListener('click', this._onSidebarOpen, true);
        if (window.ResizeObserver) {
            this._resizeObserver = new ResizeObserver(() => this._initWhenReady());
            this._resizeObserver.observe(this);
        }
    }

    disconnectedCallback() {
        document.removeEventListener('click', this._onSidebarOpen, true);
        if (this._resizeObserver) this._resizeObserver.disconnect();
    }

    _onSidebarOpen(event) {
        const toggle = event.target.closest('.sidebarBlock-heading, [data-sidebar-collapse], .sidebarBlock-slide__content-close');
        if (toggle) requestAnimationFrame(() => this._initWhenReady(true));
    }

    _initWhenReady(force) {
        if (this._initialized && !force) return;
        if (this.offsetWidth < 1) return;
    ...
    rangeSliderPrice() {
        ...
        if (!lowerSlider || !upperSlider || numberS.length < 2) return;

        const containerWidth = this.offsetWidth || this.querySelector('.facets__price--slide')?.clientWidth || 1;
        let slide = Math.max(1, Math.floor(Number(lowerSlider.max) / 100 * (22 / (containerWidth / 100))));
    ...
    updateDisplay(value1, value2) {
        ...
        if (!range || !priceSlideRangeContainer) return;

        const max = range.max;
        const width = priceSlideRangeContainer.clientWidth || this.offsetWidth;
        if (width < 1) return;

        const leftSpace = (parseInt(value1, 10) / parseInt(max, 10)) * width + 'px'
        const rightSpace = (width - (parseInt(value2, 10) / parseInt(max, 10)) * width) + 'px'
...
// Walaa 2-3-2025 commented original filter code to apply new code to fix changing grid when choosing filter issu 
